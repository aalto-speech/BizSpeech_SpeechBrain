...
test_set1 = pd.read_excel(
    "metadata/test_set1.xlsx", index_col="eventId", sheet_name="Sheet1", keep_default_na=False)
test_set2 = pd.read_excel(
    "metadata/test_set2.xlsx", index_col="eventId", sheet_name="Sheet1", keep_default_na=False)
d_test1 = test_set1.to_dict(orient='index')
d_test2 = test_set2.to_dict(orient='index')
d_test2.update(d_test1)
json.dump(d_test2, open("metadata/BizSpeech_MetaData.json", "w"), sort_keys=True)

test_set1_sheets = pd.read_excel(
    "metadata/test_set1.xlsx", sheet_name=["setA", "setB", "setC"], usecols=["eventId"], squeeze=True)
test_set2_list = pd.read_excel(
    "metadata/test_set2.xlsx", sheet_name="setA", usecols=["eventId"], squeeze=True).to_list()
...
json.dump(processed_dict, open(
    "metadata/cloudasr_testset_lists.json", "w"), sort_keys=True)
